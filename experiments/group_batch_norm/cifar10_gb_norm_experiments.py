...
from experiments.group_batch_norm.gb_norm_metadata_providers import \
    GB_NORM_EXPERIMENTS_BASE_METADATA_C10
...
from sources.models.cifar10_lda.cifar10_resnet_18_keras_model_template import \
    Cifar10LdaResnet18KerasModelTemplate
...
def run_cifar10_gb_norm_experiment():
    base_dir = Path(__file__).parent.parent.parent
    root_data_dir = base_dir / "data"

    for GN in [True, False]:
        for concentration in DEFAULT_CONCENTRATIONS_CIFAR10:
            model_template = Cifar10LdaResnet18KerasModelTemplate(DEFAULT_SEED, group_norm=GN)
            ...
            SimulateExperiment.start_experiment(
                f"Cifar10_Lda_{concentration}_Fedavg_ResNet18_{'GN' if GN else 'BN'}",
                model_template,
                dataset_factory,
                strategy_provider=None,
                strategy_provider_list=[fed_avg],
                optimizer_list=[model_template.get_optimizer()],
                experiment_metadata_list=[GB_NORM_EXPERIMENTS_BASE_METADATA_C10()],
                base_dir=base_dir,
                runs_per_experiment=2,
                centralised_evaluation=True,
                aggregated_evaluation=False,
