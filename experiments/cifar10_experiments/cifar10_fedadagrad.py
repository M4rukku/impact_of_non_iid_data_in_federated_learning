...
from sources.flwr.flwr_strategies.full_evaluation_strategy_providers import full_eval_fed_adagrad_strategy_provider
...
def run_cifar10_fedadagrad():
    ...
    for concentration in DEFAULT_CONCENTRATIONS_CIFAR10[1:]:
        ...
        conc = DEFAULT_CONCENTRATIONS_CIFAR10[1]
        spl = pgr.strategy_provider_list[1:] if concentration == conc else pgr.strategy_provider_list
        opl = pgr.optimizer_list[1:] if concentration == conc else pgr.optimizer_list
        eml = pgr.experiment_metadata_list[1:] if concentration == conc else pgr.experiment_metadata_list

        SimulateExperiment.start_experiment(
            f"Cifar10_Lda_{concentration}_Fedadagrad_HS",
            model_template,
            dataset_factory,
            strategy_provider=None,
            strategy_provider_list=spl,
            optimizer_list=opl,
            experiment_metadata_list=eml,
            base_dir=base_dir,
