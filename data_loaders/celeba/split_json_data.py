...
def split_json_and_save(
    ...
    for path_to_json in paths_to_json:
        with open(path_to_json, "r") as f:
            ...
            print(f"Starting Processing ${num_users} users. This might take a while.")

            for user_idx, u in enumerate(users_list):
                if report_count >= reporting:
                    print(f"Processed ${report_users_count} / ${num_users} users.")
                    report_count = 0
                ...
                for split_id, (dataset, fraction) in enumerate(list_datasets):
                    ...
                    if (
                        ...
                        end_idx = num_samples
                    data["x"] = [
                        Image.open(image_dir / imgPath).resize((CELEBA_IMAGE_SIZE, CELEBA_IMAGE_SIZE)).convert('RGB')
                        for imgPath in x[start_idx:end_idx]
                    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Splits a LEAF Celeba train dataset into 
    ...
    parser.add_argument(
        ...
        help="""Root directory to JSON files containing the 
                generated trainset for LEAF Celeba.""",
    )
    parser.add_argument(
        "--val_frac",
