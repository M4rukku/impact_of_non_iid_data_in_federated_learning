...
from sources.flwr_parameters.saving_parameters import pickle_parameters_to_file, \
    create_round_based_evaluation_metrics_filename, append_data_to_file, create_evaluation_metrics_filename
...
class EvaluationMetricsLoggingStrategyDecorator(BaseStrategyDecorator):
    ...
    def __init__(self, strategy: flwr.server.strategy.Strategy,
                 metrics_logging_folder: PathLike[str],
    ...
    def aggregate_evaluate(self, rnd: int, results: List[Tuple[ClientProxy, EvaluateRes]],
        ...
        logging.warning("Aggregate Evaluate")
        accuracies = None
        ...
        if accuracies is not None:
            ...
            append_data_to_file(create_evaluation_metrics_filename(self.metrics_logging_folder,
                                                                   self.experiment_identifier),
                                str(accuracy_aggregated))

        metric_parameters = list(map(lambda p: p[1].metrics, results))
        logging.warning(str(metric_parameters))
        pickle_parameters_to_file(create_round_based_evaluation_metrics_filename(rnd,
