...
def download_celeba_dataset():
    celeba_download_path = Path(__file__).parent / "leaf_root" / "data" / "celeba" / "data" / "raw"
    ...
    if celeba_unzip_dir_path.exists():
        ...
        gdown.download("https://drive.google.com/uc?id=1SCQmbKY9-zQ7BoQHnkXdW9YrSlnD27xU",
...
if __name__ == '__main__':
    dataset_identifier_parser = argparse.ArgumentParser(description='Select which datasets you wish to download.')

    dataset_identifiers = ["Celeba", "Femnist", "Shakespeare"]

    for dataset_identifier in dataset_identifiers:
        dataset_identifier_parser.add_argument(f'--{dataset_identifier.lower()}',
                                               action='store_true',
    ...
    if any(args.values()):
        print("Downloading the following datasets:",
              ", ".join([dataset_id for dataset_id, val in args.items() if val]))
        print("This might take a while...")
        print("")
    else:
        print("No Dataset to download selected. Please ask for help (-h) to see which datasets may be downloaded and "
